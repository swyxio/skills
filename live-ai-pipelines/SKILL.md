...
It applies to document extraction, knowledge graphs, story bibles, batch analysis, ETL, report generation, indexing, evals, and agent workflows. It is provider-neutral, but includes concrete OpenAI and OpenRouter patterns.
...
For each stage define its input hash, output schema, item key, expected count, retry policy, and what may be shown before the stage completes. Fan-out stages should emit progress per item; global synthesis stages should expose a provisional draft but publish only after validation.
...
The status snapshot is replaceable and convenient for polling. The event journal is append-only and replayable. The result/cache layer is the source of completed work; the UI is a projection, not the source of truth.
...
For OpenAI, use Structured Outputs with the official SDK streaming helpers when available; the SDK can expose progressive snapshots/parsed content. For OpenRouter, use `response_format.type = "json_schema"`, `strict: true`, and `stream: true` only with models/providers that support the parameter. Require the parameter during routing when the provider offers that control. Record requested and actual provider/model, generation ID, schema hash, finish reason, and validation status.
...
On restart, derive completed work from canonical artifacts and/or idempotency keys. Do not trust a progress counter by itself. Handle duplicate delivery, missing outputs, malformed streams, cancellation, and provider failures explicitly.
...
- process termination during an item, batch, and snapshot publish;
...
- a provider that ignores structured-output parameters;
...
Measure stage latency, queue time, first-byte time, item completion time, total tokens, retries, cache hits, validation failures, provider/model, and cost where available. Report elapsed times for major stages and batches.
